...
class DiffusionDDIM(object):
    ...
    def loss(self, x0, t, model, model_kwargs={}, noise=None, weight = None, use_div_loss= False, loss_mask=None):
        ...
        if self.loss_type in ['kl', 'rescaled_kl']:
            ...
            if loss_mask is not None:  # [2, 16, 1, 152, 96] b,f,c,h,w
                ...
                loss = ((out - target).pow(1 if self.loss_type.endswith('l1') else 2).abs()*loss_mask).flatten(1).mean(dim=1)
            ...
            # coherence_loss
            if self.mean_type == "v":
                pred_x0 = _i(self.sqrt_alphas_cumprod, t, xt) * xt - _i(self.sqrt_one_minus_alphas_cumprod, t, xt) * out
            elif self.mean_type == "x0":
                pred_x0 = out
            elif self.mean_type == "eps":
                pred_x0 = (xt - _i(self.sqrt_one_minus_alphas_cumprod, t, xt) * out) / _i(self.sqrt_alphas_cumprod, t, xt)
            
            pred_diff = pred_x0[:, :, 1:, :, :] - pred_x0[:, :, :-1, :, :]
            real_diff = x0[:, :, 1:, :, :] - x0[:, :, :-1, :, :]
            coherence_loss = (pred_diff - real_diff).pow(1 if self.loss_type.endswith('l1') else 2).abs().flatten(1).mean(dim=1)
            coherence_weight = 0.1
            
            # total loss
            loss = loss + loss_vlb + coherence_weight * coherence_loss
