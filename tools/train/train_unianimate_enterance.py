    ...
    for step in range(resume_step, cfg.num_steps + 1): 
        ...
        for partial_key in partial_keys:
            ...
            if cfg.use_fsdp:
                ...
                #             torch.save(batch, f'./dirty_data/batch_data_{step}.pt')
                ...
                scheduler.step()
                # else:
                #     print(f"Skipping optimizer step at step {step} due to NaN or Inf in gradients.")
            ...
            #             print(f"Parameter index {idx} ({name}) did not receive gradient.")
        ...
        if step == cfg.num_steps or step % cfg.save_ckpt_interval == 0 or step == resume_step:
            if cfg.use_ema:
                local_ema_model_path = osp.join(cfg.log_dir, f'checkpoints/{current_time}/ema_{step:08d}_rank{cfg.rank:04d}.pth')
                save_dict = {
            ...
            if cfg.rank == 0:
                ...
                local_model_path = osp.join(cfg.log_dir, f'checkpoints/{current_time}/non_ema_{step:08d}.pth')
