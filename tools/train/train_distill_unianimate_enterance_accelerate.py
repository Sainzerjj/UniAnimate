...
from tools.modules.diffusions.ddim_utils import DDIMSolver, scalings_for_boundary_conditions, append_dims
...
def train_distill_unianimate_entrance_accelerate(cfg_update,  **kwargs):
...
def worker(cfg):
    ...
    clip_encoder.model.to(dtype=weight_dtype, device=accelerator.device)
    with torch.no_grad():
    ...
    student_model = MODEL.build(cfg.UNet)
    target_model = MODEL.build(cfg.UNet)
    teacher_model = MODEL.build(cfg.UNet)
    ...
    if cfg.checkpoint_model:
        ...
        student_status = student_model.load_state_dict(state_dict, strict=True)
        teacher_status = teacher_model.load_state_dict(state_dict, strict=True)
        target_status = target_model.load_state_dict(state_dict, strict=True)
        logging.info('Load model from {} with status {} on step {}'.format(cfg.checkpoint_model, student_status, resume_step))
    ...
    whole_params_num = len(list(student_model.parameters())) # 1544
    ...
    alpha_schedule = diffusion.sqrt_alphas_cumprod
    sigma_schedule = diffusion.sqrt_one_minus_alphas_cumprod
    solver = DDIMSolver(diffusion.alphas_cumprod.numpy(), 
                        timesteps = cfg.Diffusion.schedule_param.num_timesteps, 
                        ddim_timesteps = cfg.num_ddim_timesteps,
                    )
    alpha_schedule = alpha_schedule.to(accelerator.device)
    sigma_schedule = sigma_schedule.to(accelerator.device)
    solver.to(device=accelerator.device, dtype=weight_dtype)
    ...
    trainable_params = list(filter(lambda p: p.requires_grad, student_model.parameters()))
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(dataloader):
            with accelerator.accumulate(student_model):
                ...
                uncond_model_kwargs = {key: (torch.zeros_like(value) if value is not None else None) for key, value in model_kwargs.items()}
                # prob = torch.rand(1)
                ...
                for partial_key in partial_keys:
                    ...
                    topk = cfg.Diffusion.schedule_param.num_timesteps // cfg.num_ddim_timesteps
                    index = torch.randint(0, cfg.num_ddim_timesteps, (batch_size,), device=video_data.device).long()
                    ...
                    c_skip_start, c_out_start = [append_dims(x, video_data.ndim) for x in [c_skip_start, c_out_start]]
                    c_skip, c_out = scalings_for_boundary_conditions(timesteps)
                    c_skip, c_out = [append_dims(x, video_data.ndim) for x in [c_skip, c_out]]
                    
                    
                    # 20.4.5. Add noise to the latents according to the noise magnitude at each timestep
                    # (this is the forward diffusion process) [z_{t_{n + k}} in Algorithm 1]
                    noisy_model_input = diffusion.q_sample(video_data, start_timesteps, noise)

                    # 20.4.6. Sample a random guidance scale w from U[w_min, w_max] and embed it
                    w = (cfg.w_max - cfg.w_min) * torch.rand((batch_size,)) + cfg.w_min
                    w = w.reshape(batch_size, 1, 1, 1)
                    w = w.to(device=video_data.device, dtype=video_data.dtype)

                    _, _, _, pred_x_0 = diffusion.p_mean_variance(noisy_model_input, start_timesteps, student_model, model_kwargs)
                    model_pred = c_skip_start * noisy_model_input + c_out_start * pred_x_0
                    
                    with torch.no_grad():
                        with torch.autocast("cuda"):
                            x_prev, _ = diffusion.ddim_sample(
                                noisy_model_input, 
                                start_timesteps, 
                                teacher_model, 
                    ...
                    with torch.no_grad():
                        with torch.autocast("cuda", dtype=weight_dtype):
                            _, _, _, pred_x_0 = diffusion.p_mean_variance(x_prev.float(), timesteps, target_model, model_kwargs)
            ...
            if global_step % cfg.save_ckpt_interval == 0 and accelerator.is_main_process:
                unwrap_model = accelerator.unwrap_model(student_model)
                local_model_path = osp.join(cfg.log_dir, f'checkpoints/{current_time}/non_ema_distill_{epoch}_{global_step:08d}.pth')
